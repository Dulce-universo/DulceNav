...
class StorageService {
  ...
  Future<void> init() async {
    ...
    await checkVersionMigration('1.8.1');
  }
  ...
  static const String _keyInstalledVersion = 'installed_version';

  String? get lastInstalledVersion => _p.getString(_keyInstalledVersion);

  Future<void> checkVersionMigration(String currentVersion) async {
    final oldVersion = lastInstalledVersion;
    if (oldVersion != currentVersion) {
      debugPrint('[StorageService] Actualización incremental detectada de "${oldVersion ?? "versión previa"}" a "$currentVersion". Preservando datos e integridad de usuario.');
      await _p.setString(_keyInstalledVersion, currentVersion);
    }
  }
  ...
  static const String _keyWeatherCachedCity = 'weather_cached_city';
  static const String _keyWeatherCachedTemp = 'weather_cached_temp';
  static const String _keyWeatherCachedCondition = 'weather_cached_condition';
  static const String _keyWeatherCachedHumidity = 'weather_cached_humidity';
  static const String _keyWeatherCachedWind = 'weather_cached_wind';
  static const String _keyWeatherCachedIconCode = 'weather_cached_icon_code';
  ...
  // ── Clima Persistente ─────────────────────
  String get weatherCachedCity => _p.getString(_keyWeatherCachedCity) ?? 'Manizales';
  double get weatherCachedTemp => _p.getDouble(_keyWeatherCachedTemp) ?? 18.0;
  String get weatherCachedCondition => _p.getString(_keyWeatherCachedCondition) ?? 'Parcialmente Nublado';
  int get weatherCachedHumidity => _p.getInt(_keyWeatherCachedHumidity) ?? 70;
  double get weatherCachedWind => _p.getDouble(_keyWeatherCachedWind) ?? 8.5;
  int get weatherCachedIconCode => _p.getInt(_keyWeatherCachedIconCode) ?? 0;
  ...
  Future<void> saveCachedWeather({
    ...
    await _p.setString(_keyWeatherCachedCity, city);
    await _p.setDouble(_keyWeatherCachedTemp, temp);
    await _p.setString(_keyWeatherCachedCondition, condition);
    await _p.setInt(_keyWeatherCachedHumidity, humidity);
    await _p.setDouble(_keyWeatherCachedWind, wind);
    await _p.setInt(_keyWeatherCachedIconCode, iconCode);
  }
