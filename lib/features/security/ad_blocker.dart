...
function ntf(){
  try{
    if(window.flutter_inappwebview && window.flutter_inappwebview.callHandler){
      window.flutter_inappwebview.callHandler('webMessage', {type: 'ad_blocked'});
    } else if(window.chrome && window.chrome.webview && window.chrome.webview.postMessage){
      window.chrome.webview.postMessage(JSON.stringify({type: 'ad_blocked'}));
    }
  }catch(e){}
}
function chk(u){
  try{
    ...
    var isB=false;
    if(BL.has(h)) isB=true;
    if(!isB){
      var p=h.split(".");
      for(var i=1;i<p.length-1;i++){
        if(BL.has(p.slice(i).join("."))){ isB=true; break; }
      }
    }
    if(isB){ ntf(); return true; }
  }catch(e){}
  return false;
}
